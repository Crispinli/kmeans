...
def initCentroids(dataSet, k):
    ...
    for i in range(1, k + 1):  # i 从 1 迭代到 k
        while True:
            index = int(np.random.uniform(0, numSamples))  # 将随机生成的随机数转换成整型
...
def kmeans(dataSet, k):
    ...
    clusterAssment = np.mat(np.zeros((numSamples, 2)))  # 第一列存这个样本点属于哪个簇，第二列存这个样本点和样本中心的误差
    for i in range(numSamples):
        clusterAssment[i, 0] = -1
    clusterChanged = True

    # step 1: 初始化 centroids 矩阵
    ...
    while clusterChanged:  # 如果已经收敛，则 clusterChanged 的值为 False
        ...
        # step 4: 更新样本中心
        print("clusterAssment before: ", clusterAssment)
